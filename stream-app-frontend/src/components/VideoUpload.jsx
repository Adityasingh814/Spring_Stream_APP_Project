...
import videoLogo from "../assets/upload-2.png";
import {
  Button,
  Card,
  Label,
  ...
  async function saveVideoToServer(video, videoMetaData) {
    ...
    try {
      ...
      let response = await axios.post(
        `http://localhost:1081/api/videos`,
        formData,
        {
          headers: {
      ...
      <Card className="flex flex-col items-center justify-center">
        ...
        <div>
          ...
          >
            ...
            {uploading && (
              <Progress color="green" progress={progress} textLabel="Uploading" size="lg" />
            )}

            {message && (
              <Alert color="success" rounded onDismiss={() => setMessage("")}>
                {message}
              </Alert>
            )}
